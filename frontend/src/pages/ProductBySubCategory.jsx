...
function ProductBySubCategory() {
    ...
    const fetchProduct = async () => {
      ...
      try {
        const res = await api.get(
          `/api/v1/category/categories/${mainCategoryName}/subCategories/${subCategoryName}/products?startIndex=0&limit=${limit}`
        );
        if (res) {
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await api.get(
        `/api/v1/category/categories/${mainCategoryName}/subCategories/${subCategoryName}/products?startIndex=${startIndex}&limit=${limit}`
      );
      if (res) {
      ...
      <div className="bg-secondary py-20">
        <div className="max-w-7xl mx-auto px-8">
          <div className="flex flex-col items-center gap-4 text-center">
            <button
              onClick={() => navigate(`/products/category/${mainCategoryName}`)}
              className="self-start flex items-center gap-2 text-primary hover:text-btn dark:hover:text-btn-dark transition-colors font-semibold"
            >
              <ArrowLeft size={20} />
              <span>Back to {mainCategoryName}</span>
            </button>
            ...
            <div>
              <p className="text-primary text-sm mb-2 opacity-70">
                {mainCategoryName} Collection
              </p>
              <h1 className="head-text">{capitalizeFirst(subCategoryName)}</h1>
            </div>
            <p className="text-primary text-lg">
              Explore our {subCategoryName} collection
            </p>
          </div>
        </div>
      </div>
        ...
        {product.length > 0 && (
          <div className="mb-6 flex items-center justify-between flex-wrap gap-4">
            ...
            <Link
              ...
              className="btn-primary bg-secondary text-primary font-semibold px-4 py-2"
            >
              View All {capitalizeFirst(mainCategoryName)} Products
            </Link>
          </div>
        )}
            ...
            {product.map((item, index) => (
              ...
              >
                ...
                <div className="p-4 flex flex-col gap-2">
                  ...
                  {item.description && (
                    <p className="text-primary text-xs opacity-70 line-clamp-2">
                      {item.description}
                    </p>
                  )}

                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-bg-secondary-dark dark:border-bg-secondary">
                  ...
                  <button
                    onClick={() =>
                      navigate(
                        `/product/category/${mainCategoryName}/${subCategoryName}/${item.slug}`
                      )
                    }
                    className="w-full mt-2 px-3 py-2 bg-btn dark:bg-btn-dark text-white text-sm font-semibold rounded-md hover:bg-hover dark:hover:bg-hover-dark transition-colors"
                  >
                    View Details
                  </button>
                </div>
          ...
          !loading && (
            <div className="flex flex-col items-center justify-center py-20 gap-6">
              ...
              <p className="text-primary text-center max-w-md">
                No products available in {subCategoryName} category yet. Check
                back soon!
              </p>
              <div className="flex gap-4">
                <button
                  onClick={() =>
                    navigate(`/products/category/${mainCategoryName}`)
                  }
                ...
                <button
                  onClick={() => navigate("/")}
                  className="px-5 py-3 border-2 border-btn dark:border-btn-dark text-btn dark:text-btn-dark rounded-full hover:bg-btn hover:dark:bg-btn-dark hover:text-white font-semibold transition-all duration-300"
                >
                  Go to Home
                </button>
              </div>
